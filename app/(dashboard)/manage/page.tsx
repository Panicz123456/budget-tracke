          ...
          )}
        {
          dataAvailable && (
            <div className="grid grid-flow-row gap-2 p-2 sm:grid-flow-row sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
              {categoriesQuery.data.map((categories: Category) => (
                <CategoriesCard categories={categories} key={categories.name} />
              ))}
            </div>
          )
        }
  ...
  return (
    ...
    >
      <div className="flex flex-col items-center gap-2 p-4">
        <span className="text-3xl" role="img">{categories.icon}</span>
